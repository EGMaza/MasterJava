    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(encontrado.isPresent()){
            ...
            out.println("      <h3>Producto encontrado: "+ encontrado.get().getNombre() +
                    " con un precio de $"+ encontrado.orElseThrow().getPrecio() +"</h3>");
            out.println("   </body>");
            out.println("</html>");
            }
        }
        else{
            resp.sendError(HttpServletResponse.SC_NOT_FOUND, "No se encontró ningún producto con el nombre " + nombre);
        }
    }
}
